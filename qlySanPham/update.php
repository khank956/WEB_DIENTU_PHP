...
if (isset($_GET["id"])) {
    $id = $_GET["id"];
    // Kiểm tra sản phẩm có trạng thái "BiXoa" là 0 (chưa bị xóa) mới cho phép cập nhật
    $checkStatusQuery = "SELECT BiXoa FROM sanpham WHERE MaSanPham = '$id'";
    $checkStatusResult = mysqli_query($connection, $checkStatusQuery);

    if ($checkStatusResult) {
        $rowStatus = mysqli_fetch_assoc($checkStatusResult);

        if ($rowStatus['BiXoa'] == 0) {
    //update sql 
...
<?php
            ...
            echo "<div class='listdel'>Sản phẩm đã bị xóa và không thể cập nhật.  <a href='/qlySanPham/index.php?mod=panel'>Quay lại</a></div>";
        }
    } else {
        echo "Error: " . $checkStatusQuery . "<br>" . mysqli_error($connection);
    }
}
?>